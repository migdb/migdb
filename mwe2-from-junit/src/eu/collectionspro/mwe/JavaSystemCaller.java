 ...
 * Java. <br />
 * This class only demonstrate a 'dir' or 'ls' within current (execution) path,
 * if no parameters are used. If parameters are used, the first one is the
 * system command to execute, the others are its system command parameters. <br />
 * To be system independent, an <b><a href=
...
public final class JavaSystemCaller {
	 ...
	 * Default is 'ls' in current directory if no parameters, or a system
	 * command (if Windows, it is automatically translated to 'dir')
	 *
	 * @param args
	 *            first element is the system command, the others are its
	 *            parameters (NOT NULL)
	 * @throws IllegalArgumentException
	 *             if one parameters is null or empty. 'args' can be empty
	 *             (default 'ls' performed then)
	 */
	public static void main(final String[] args) {
		String anOutput = "";
		if (args.length == 0) {
			anOutput = Exec.execute("sed", "-i", "-e", "s/import/IMPORT/g", "./model/test.mmfull");
		} else {
	...
	public static class StreamGobbler extends Thread {
		...
		private String sysOutPrefix;
		private StringBuffer output = new StringBuffer();

		StreamGobbler(final InputStream anIs, final String sysOutPrefix) {
			this.is = anIs;
			this.sysOutPrefix = sysOutPrefix;
		}
		...
		public final void run() {
			try {
				...
				while ((line = br.readLine()) != null) {
					System.out.println(this.sysOutPrefix + line);
	...
	public static final class Exec {
		 ...
		 *
		 * @param aCommand
		 *            system command to be executed (must not be null or empty)
		 * @param someParameters
		 *            parameters of the command (must not be null or empty)
		 * @return final output (stdout only)
		 */
		public static String execute(final String aCommand, final String... someParameters) {
			String output = "";
			try {
				ExecEnvironmentFactory anExecEnvFactory = getExecEnvironmentFactory(aCommand,
						someParameters);
	...
	public abstract static class ExecEnvironmentFactory {
		 ...
		 * Means that a command might be translated, if it does not fit the
		 * right OS ('dir' => 'ls' on unix)
		 *
	...
	public static final class WindowsExecEnvFactory extends ExecEnvironmentFactory {
		 ...
		 * played. <br />
		 * Any command not from windows will be translated in its windows
	...
	public static final class UnixExecEnvFactory extends ExecEnvironmentFactory {
		 ...
		 * played. <br />
		 * Any command not from Unix will be translated in its Unix equivalent
		 * if possible.
		 *
		 * @param aCommand
		 *            system command to be executed (must not be null or empty)
